...
function Transmission() {
    ...
    function saveClearNote() {
        localStorage.setItem(
            'notesTran', JSON.stringify([...notes, {id: nanoid(), date: new Date().toLocaleDateString(), text: note, showStr: false}])
        );
        setNotes( JSON.parse(localStorage.getItem('notesTran')) );
    ...
    function btnDel(id) {
        localStorage.setItem( 'notesTran', JSON.stringify(notes.filter(noteObj => noteObj.id !== id)) );
        setNotes( JSON.parse(localStorage.getItem('notesTran')) );
    }
